...
START_WEBSERVER() {
  ...
  case $VAR_PYTHON_VERSION in
	2)
      # Python 2's SimpleHTTPServer binds to 0.0.0.0 by default when only port is given.
      # No explicit address needed here.
      (cd $VAR_SCRIPTLOC/log; $VAR_PYTHON_EXEC -m SimpleHTTPServer $1 &) &> /dev/null  
    ;;
    3)
      # Python 3's http.server needs -b 0.0.0.0 to bind to all interfaces.
      (cd $VAR_SCRIPTLOC/log; $VAR_PYTHON_EXEC -m http.server $1 -b 0.0.0.0 &) &> /dev/null
    ;;
  esac
}
